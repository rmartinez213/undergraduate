...
public class AVLTree<E extends Comparable<E>> extends BST<E> {
	...
	public E find(int k){
		System.out.println(find2(k, root));

		return null;
	}

	public int find2(int k, TreeNode<E> root){
		
		if(root.left != null && (int)root.left.element < k ){
			k = (int)root.left.element;
		}
		if(root.right != null && (int)root.right.element < k){
			k = (int)root.right.element;
		}
